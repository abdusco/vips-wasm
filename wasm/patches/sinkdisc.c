...
wbuffer_new(Write *write)
{
	...
	/* WASM single-threaded build: no background write thread.
	 * Writes happen synchronously in wbuffer_flush; running=FALSE
	 * so wbuffer_free skips the thread kill/join.
	 */
	wbuffer->running = FALSE;
...
wbuffer_flush(Write *write)
{
	 ...
	 * In the WASM single-threaded build the previous wbuffer_flush wrote
	 * buf_back synchronously and pre-signalled done, so this is instant.
	 */
	...
	/* WASM single-threaded: write inline — no background thread.
	 * Wait for all compute workers to release the buffer, then write it
	 * synchronously and signal done so the next flush can proceed.
	 */
	vips_semaphore_downn(&write->buf->nwrite, 0);
	wbuffer_write(write->buf);
	vips_semaphore_up(&write->buf->done);
	return write->buf->write_errno ? -1 : 0;
}
