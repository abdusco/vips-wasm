...
vips_threadset_new(int max_threads)
{
	...
	/* WASM single-threaded build: do not pre-create OS threads.
	 * vips_threadset_run executes tasks inline instead.
	 */
...
{
	/* WASM single-threaded build: execute tasks synchronously inline.
	 * The threadpool workers (vips_thread_main_loop) run to completion
	 * here; the wbuffer write path in sinkdisc.c is patched to be
	 * synchronous so there is no blocking on semaphores.
	 */
	func(data, NULL);
	return 0;
}
